...
module.exports = {
    ...
    async execute(message) {
        ...
        if (!originalMessage.content.startsWith('Hello')) {
            return;
        }
        ...
        if (originalMessage.author.id === message.client.user.id) {
            ...
            while (currentMessage.reference) {
                ...
                const sender = currentMessage.author.id === message.client.user.id ? 'Bot' : 'User';
                threadMessages.push(`${sender}: ${currentMessage.content}`);
            }
